...
不过，最新实现与上一版文档最大的不同在于：它已经不再维护一个显式的
\texttt{unordered\_map}-式重排序缓冲区（reorder buffer）。
当前代码把“乱序完成、顺序提交”这层语义折叠进了一个
\textbf{按输入顺序排队的 \texttt{future} 队列}里。于是整体骨架更准确地写成
\[
    \texttt{input}
    \longrightarrow
    \texttt{dispatch}
    ...
    \texttt{single writer}.
\]
如果借用处理器体系结构（computer architecture）的语言，这更像“有序提交
（ordered commit）”而不是“显式按 index 回填的重排序表”。
之所以仍保留本章的历史标题，是因为它表达的抽象问题没有变：输入顺序必须保真，工作线程可以乱序完成，
而文件输出必须重新回到确定性的提交顺序。本章就围绕这层新语义，重写 pipeline 的理论动机、控制流与工程边界。
...
\]
旧版实现的思路是“先接受乱序完成，再按 \texttt{testcase\_index} 重排”；新版实现则走了另一条路：
...
\[
    ...
    \texttt{commit}(N-1).
\]
这是一种更收缩的状态机：系统不再需要显式维护“谁已经完成但不能提交”的散列表，
而是用队列顺序天然表达“谁有资格先提交”。

当然，这种设计也不是免费午餐。它把原本的“显式重排复杂度”换成了“队首阻塞
...
这不是 profiling 精确值，而是一条更诚实的系统结论：新实现仍然是\emph{有界}的，但边界来自
``有界 future admission''，而不是旧版那种显式完成包缓存。
...
这一小节的标题保留了“重排序缓冲区”这个历史说法，但必须先说清楚：
\textbf{当前实现已经没有单独的 \texttt{reorder\_buffer\_} 容器。}
新的顺序恢复机制，实际上是“按输入顺序排队的 future 提交队列 + 单 writer 顺序 \texttt{get()}”。
从抽象效果看，它仍然实现了 ordered commit；从数据结构看，它比旧版更收缩。
...
\[
    ...
    \end{aligned}
\]
这里最值得注意的变化是：\textbf{输出阶段接收的不是已经完成的 \texttt{OutputPacket}，而是
...
但当前 writer \textbf{并不再依赖该 index 做重排}；它更像一种审计字段（audit field）与调试字段。

\paragraph{第三步：future 在生成后立刻进入输出阶段，而不是先在主线程积压。}
这是新版并发模型最本质的变化之一。当前 \texttt{run()} 并没有维护额外的
...
\]
因此，主线程不再手工轮询一串 future 来决定“哪一个先 ready”，而是把这个顺序资格整体转交给输出阶段。
这让主线程的责任进一步收缩成“顺序读取、顺序投递”。

\paragraph{第四步：\texttt{FutureQueueOutputStage} 用 FIFO future 队列表达 ordered commit。}
...
\]
所以，当前的“顺序恢复”不是通过比较 \texttt{packet.testcase\_index} 完成的，
...
这与旧版“先接纳所有已完成 packet，再按 index 刷出连续前缀”的思路不同。
旧版偏向乱序完成后显式回填；新版偏向\emph{有序提交窗口}。
...
并发代码最危险的地方，往往不是锁多不多，而是状态与例外太多。当前新版实现能保持可分析性，
正是因为它把关键状态压缩成了：一个全局线程池、一个有界 future 队列、一个 writer 线程、
一组关闭标志和一个异常指针。状态空间越小，正确性才越容易被真正推理，而不是靠祈祷。

\subsection{当前实现的边界与可演化方向}

在新的 future-queue 并发模型下，\texttt{pipeline.cpp} 的结构已经明显更整洁；
但边界也同样更加清晰了。这些边界不是“将来一定要修”的 bug 列表，
更像是后续演化时必须显式作答的设计问题。
...
\paragraph{第二，ordered commit 天然带来 head-of-line blocking。}
...
那才值得重新引入显式完成包缓存、按 index 提交的真正 reorder buffer。
...
这很值得记一笔，因为它说明代码语义已经发生了实质性变化。
旧模型下 index 是重排主键；新模型下 index 只是“这个 packet 对应谁”的事实记录。
...
与旧版文档相比，当前实现最关键的变化不是“线程更多了”，而是\textbf{顺序恢复机制换了形}：
显式重排序缓冲区被收缩为按输入顺序提交的 future 队列，writer 在固定提交点上执行 \texttt{future.get()}，
由此实现 ordered commit。
