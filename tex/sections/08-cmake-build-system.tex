...
\[
    \mathcal{B}:
    \{\texttt{main.cpp},\texttt{pipeline.cpp},\texttt{io.cu},\texttt{kernels.cu}\}
    \longrightarrow
...
\subsection{工具链选择与构建契约}

\paragraph{最小语言声明：为什么从 \texttt{LANGUAGES NONE} 开始。}
...
\paragraph{两条编译路径：原生 CUDA 语言与 \texttt{nvcc} driver mode。}
...
\paragraph{输出路径契约：把 artifact 放在项目根 \texttt{build/}。}
...
\paragraph{包含目录与依赖边界：当前只有项目内头文件。}
...
\subsection{可复现实验的操作模型}

\paragraph{构建命令：可复现实验的最小仪式。}
...
\subsection{工程审查与后续演化}

\paragraph{复杂度审查：当前 CMake 的好味道与坏味道。}
...
\paragraph{与论文构建的关系：两个构建系统，不要互相污染。}
...
\paragraph{安装、导出与包边界：现在不做，但要知道为什么。}
